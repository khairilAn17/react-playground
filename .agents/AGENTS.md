...
- **Dedicated Component Packages**: Keep distinct UI primitives in separate directories inside `src/components/`:
  ...
  - `src/components/pageLayout/`: `PageLayout` compound layout system (`Header`, `Content`, `Section`, `Steps`, `StickyFooter`, `Skeleton`).
...
- **Decoupled Primitives**: Keep general UI primitives (`Card`, `ExpandableSearch`, `Grid`) decoupled from compound wrappers like `PageLayout` so they can be freely reused in drawers, dialogs, panels, or standalone views.
