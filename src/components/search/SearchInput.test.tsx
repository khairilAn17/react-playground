...
function ControlledSearch() {
  ...
  return <SearchInput value={val} onChange={(next) => setVal(next)} placeholder="Search team" />
}
...
describe('SearchInput Primitive', () => {
  ...
  it('updates controlled value and triggers onChange', () => {
    const handleChange = vi.fn()
    render(<SearchInput value="test" onChange={handleChange} placeholder="Search..." />)

    const input = screen.getByPlaceholderText('Search...')
    expect(input).toHaveValue('test')

    fireEvent.change(input, { target: { value: 'new query' } })
    expect(handleChange).toHaveBeenCalledWith('new query', expect.anything())
  })
